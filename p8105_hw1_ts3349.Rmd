...
Descriptive analysis of penguins data:
```{r problem 2 part 1}
...
levels(penguins$sex)
min(penguins$year, na.rm = TRUE)
max(penguins$year, na.rm = TRUE)
...
```
The penguins data contains 8 different variables including species(factor), island(factor), bill_length_mm(numeric), bill_depth_mm(numeric), flipper_length_mm(numeric), body_mass_g(numeric), sex(factor), and year(numeric).

There are 3 different species, `r levels(penguins$species)` of penguins.  There are 3 different island locations of the penguins, `r levels(penguins$island)`.  There are 2 different sexes of the penguins, `r levels(penguins$sex)`.
